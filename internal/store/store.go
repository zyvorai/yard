...
func Open(dsn string) (*Store, error) {
	if dsn == "" {
		dsn = "file:estate.db?_pragma=busy_timeout(5000)&_pragma=foreign_keys(ON)"
	}
...
func (s *Store) CreateOrganization(ctx context.Context, name, slug string) (*model.Organization, error) {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO organizations(id,name,slug,created_at) VALUES(?,?,?,?)`, o.ID, o.Name, o.Slug, o.CreatedAt.Format(time.RFC3339Nano))
	return o, err
}
...
func (s *Store) CreateUser(ctx context.Context, u *model.User) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO users(id,organization_id,email,display_name,role,password_hash,created_at) VALUES(?,?,?,?,?,?,?)`,
...
func (s *Store) UserByEmail(ctx context.Context, email string) (*model.User, error) {
	row := s.DB.QueryRowContext(ctx, `SELECT id,organization_id,email,display_name,role,password_hash,created_at FROM users WHERE email=?`, strings.ToLower(email))
	return scanUser(row)
}

func (s *Store) UserByID(ctx context.Context, id string) (*model.User, error) {
	row := s.DB.QueryRowContext(ctx, `SELECT id,organization_id,email,display_name,role,password_hash,created_at FROM users WHERE id=?`, id)
	return scanUser(row)
}
...
func (s *Store) CreateSession(ctx context.Context, userID string, ttl time.Duration) (*model.Session, error) {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO sessions(token,user_id,expires_at) VALUES(?,?,?)`, sess.Token, sess.UserID, sess.ExpiresAt.Format(time.RFC3339Nano))
	return sess, err
}
...
func (s *Store) UpsertSite(ctx context.Context, site *model.Site) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO sites(id,organization_id,name,kind,address,latitude,longitude,timezone,created_at)
...
func (s *Store) ListSites(ctx context.Context, orgID string) ([]model.Site, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT id,organization_id,name,kind,address,latitude,longitude,timezone,created_at FROM sites WHERE organization_id=? ORDER BY name`, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) SiteByName(ctx context.Context, orgID, name string) (*model.Site, error) {
	row := s.DB.QueryRowContext(ctx, `SELECT id,organization_id,name,kind,address,latitude,longitude,timezone,created_at FROM sites WHERE organization_id=? AND name=?`, orgID, name)
	var st model.Site
	var created string
	var lat, lng sql.NullFloat64
...
func (s *Store) UpsertAsset(ctx context.Context, a *model.Asset) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO assets(id,organization_id,site_id,name,external_ref,kind,status,health,manufacturer,model,serial,latitude,longitude,last_seen_at,stale_after_sec,metadata,created_at,updated_at)
VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
...
func (s *Store) TouchAsset(ctx context.Context, id string, lat, lng *float64, health string) error {
	_, err := s.DB.ExecContext(ctx, `UPDATE assets SET last_seen_at=?, latitude=COALESCE(?, latitude), longitude=COALESCE(?, longitude), health=CASE WHEN ? != '' THEN ? ELSE health END, updated_at=? WHERE id=?`,
		now(), lat, lng, health, health, now(), id)
	return err
}

func (s *Store) SetAssetHealth(ctx context.Context, id, health string) error {
	_, err := s.DB.ExecContext(ctx, `UPDATE assets SET health=?, updated_at=? WHERE id=?`, health, now(), id)
	return err
}

func (s *Store) RefreshStale(ctx context.Context, orgID string) (int, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT id, last_seen_at, stale_after_sec, health FROM assets WHERE organization_id=?`, orgID)
	if err != nil {
		return 0, err
	}
	nowT := time.Now().UTC()
...
func (s *Store) ReplaceCapabilities(ctx context.Context, assetID string, caps []model.Capability) error {
	_, _ = s.DB.ExecContext(ctx, `DELETE FROM capabilities WHERE asset_id=?`, assetID)
	for _, c := range caps {
		if c.ID == "" {
			c.ID = idgen.New("cap")
		}
		if _, err := s.DB.ExecContext(ctx, `INSERT INTO capabilities(id,asset_id,name,kind,unit,min,max,writable) VALUES(?,?,?,?,?,?,?,?)`,
...
func (s *Store) ListCapabilities(ctx context.Context, assetID string) ([]model.Capability, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT id,asset_id,name,kind,unit,min,max,writable FROM capabilities WHERE asset_id=?`, assetID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) InsertObservation(ctx context.Context, o *model.Observation) (bool, error) {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO observations(id,organization_id,asset_id,capability,value,unit,quality,source,observed_at,received_at,dedupe_key)
...
func (s *Store) InsertEvent(ctx context.Context, e *model.Event) (bool, error) {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO events(id,organization_id,asset_id,site_id,kind,severity,title,body,dedupe_key,created_at)
...
func (s *Store) ListEvents(ctx context.Context, orgID string, limit int) ([]model.Event, error) {
	...
	rows, err := s.DB.QueryContext(ctx, `SELECT id,organization_id,asset_id,site_id,kind,severity,title,body,dedupe_key,created_at FROM events WHERE organization_id=? ORDER BY created_at DESC LIMIT ?`, orgID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CreateIncident(ctx context.Context, inc *model.Incident) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO incidents(id,organization_id,asset_id,site_id,title,severity,status,owner,summary,resolution,opened_at,resolved_at)
...
func (s *Store) UpdateIncident(ctx context.Context, inc *model.Incident) error {
	_, err := s.DB.ExecContext(ctx, `UPDATE incidents SET title=?, severity=?, status=?, owner=?, summary=?, resolution=?, resolved_at=? WHERE id=? AND organization_id=?`,
...
func (s *Store) GetIncident(ctx context.Context, orgID, id string) (*model.Incident, error) {
	row := s.DB.QueryRowContext(ctx, `SELECT id,organization_id,asset_id,site_id,title,severity,status,owner,summary,resolution,opened_at,resolved_at FROM incidents WHERE organization_id=? AND id=?`, orgID, id)
...
func (s *Store) OpenIncidentByDedupe(ctx context.Context, orgID, title string, assetID *string) (*model.Incident, bool, error) {
	...
	if assetID != nil {
		q += ` AND asset_id=?`
		row := s.DB.QueryRowContext(ctx, q, orgID, title, *assetID)
...
func (s *Store) CreateWorkOrder(ctx context.Context, wo *model.WorkOrder) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO work_orders(id,organization_id,asset_id,site_id,incident_id,title,kind,priority,status,assignee,notes,due_at,created_at,updated_at)
...
func (s *Store) UpdateWorkOrder(ctx context.Context, wo *model.WorkOrder) error {
	...
	_, err := s.DB.ExecContext(ctx, `UPDATE work_orders SET title=?, kind=?, priority=?, status=?, assignee=?, notes=?, incident_id=?, updated_at=? WHERE id=? AND organization_id=?`,
...
func (s *Store) GetWorkOrder(ctx context.Context, orgID, id string) (*model.WorkOrder, error) {
	row := s.DB.QueryRowContext(ctx, `SELECT id,organization_id,asset_id,site_id,incident_id,title,kind,priority,status,assignee,notes,due_at,created_at,updated_at FROM work_orders WHERE organization_id=? AND id=?`, orgID, id)
	return scanWO(row)
}
...
func (s *Store) CreateConnector(ctx context.Context, c *model.Connector) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO connectors(id,organization_id,name,kind,status,endpoint,token_hint,token_hash,actions,config,last_sync_at,created_at)
...
func (s *Store) ListConnectors(ctx context.Context, orgID string) ([]model.Connector, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT id,organization_id,name,kind,status,endpoint,token_hint,actions,config,last_sync_at,created_at FROM connectors WHERE organization_id=? ORDER BY name`, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) ConnectorByTokenHash(ctx context.Context, hash string) (*model.Connector, error) {
	row := s.DB.QueryRowContext(ctx, `SELECT id,organization_id,name,kind,status,endpoint,token_hint,token_hash,actions,config,last_sync_at,created_at FROM connectors WHERE token_hash=?`, hash)
	var c model.Connector
	var last sql.NullString
	var created string
...
func (s *Store) TouchConnector(ctx context.Context, id string) error {
	_, err := s.DB.ExecContext(ctx, `UPDATE connectors SET last_sync_at=?, status='connected' WHERE id=?`, now(), id)
	return err
}
...
func (s *Store) CreateAction(ctx context.Context, a *model.ActionRequest) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO action_requests(id,organization_id,asset_id,connector_id,action,idempotency_key,status,payload,result,expires_at,created_at,completed_at)
...
func (s *Store) ListActions(ctx context.Context, orgID string) ([]model.ActionRequest, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT id,organization_id,asset_id,connector_id,action,idempotency_key,status,payload,result,expires_at,created_at,completed_at FROM action_requests WHERE organization_id=? ORDER BY created_at DESC LIMIT 100`, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) CompleteAction(ctx context.Context, id, status, result string) error {
	_, err := s.DB.ExecContext(ctx, `UPDATE action_requests SET status=?, result=?, completed_at=? WHERE id=?`, status, result, now(), id)
	return err
}
...
func (s *Store) CreateAutomation(ctx context.Context, a *model.Automation) error {
	...
	_, err := s.DB.ExecContext(ctx, `INSERT INTO automations(id,organization_id,name,enabled,trigger_kind,capability,operator,threshold,action,created_at)
...
func (s *Store) ListAutomations(ctx context.Context, orgID string) ([]model.Automation, error) {
	rows, err := s.DB.QueryContext(ctx, `SELECT id,organization_id,name,enabled,trigger_kind,capability,operator,threshold,action,created_at FROM automations WHERE organization_id=? ORDER BY name`, orgID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) SetAutomationEnabled(ctx context.Context, orgID, id string, enabled bool) error {
	_, err := s.DB.ExecContext(ctx, `UPDATE automations SET enabled=? WHERE organization_id=? AND id=?`, boolInt(enabled), orgID, id)
	return err
}

func (s *Store) Audit(ctx context.Context, orgID, actor, action, object, detail string) error {
	_, err := s.DB.ExecContext(ctx, `INSERT INTO audit_log(id,organization_id,actor,action,object,detail,created_at) VALUES(?,?,?,?,?,?,?)`,
...
func (s *Store) ListAudit(ctx context.Context, orgID string, limit int) ([]model.AuditEntry, error) {
	...
	rows, err := s.DB.QueryContext(ctx, `SELECT id,organization_id,actor,action,object,detail,created_at FROM audit_log WHERE organization_id=? ORDER BY created_at DESC LIMIT ?`, orgID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Store) Overview(ctx context.Context, orgID string) (*model.Overview, error) {
	...
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM assets WHERE organization_id=?`, orgID).Scan(&ov.AssetsTotal)
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM assets WHERE organization_id=? AND health='healthy'`, orgID).Scan(&ov.AssetsHealthy)
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM assets WHERE organization_id=? AND health='degraded'`, orgID).Scan(&ov.AssetsDegraded)
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM assets WHERE organization_id=? AND health='critical'`, orgID).Scan(&ov.AssetsCritical)
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM assets WHERE organization_id=? AND health='stale'`, orgID).Scan(&ov.AssetsStale)
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM incidents WHERE organization_id=? AND status IN ('open','ack')`, orgID).Scan(&ov.OpenIncidents)
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM work_orders WHERE organization_id=? AND status NOT IN ('done','cancelled')`, orgID).Scan(&ov.OpenWorkOrders)
	_ = s.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM connectors WHERE organization_id=? AND status='connected'`, orgID).Scan(&ov.ActiveConnectors)
	rows, err := s.DB.QueryContext(ctx, `SELECT kind, COUNT(*) FROM assets WHERE organization_id=? GROUP BY kind`, orgID)
	if err == nil {
		defer rows.Close()
		for rows.Next() {
			var k string
